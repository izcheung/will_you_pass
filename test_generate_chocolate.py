...
class TestGenerateChocolate(unittest.TestCase):
    ...
    @patch('random.sample', side_effect=lambda population, k: [(0, 6)] if k == 1 else [(0, 6), (0, 0), (4, 6), (4, 2), (2, 6)])
    def test_generate_chocolate(self, _):
        # Test Case 1: Normal case with obstacles and player
        player = {'name': 'Irene', 'level': 1, 'intelligence': 0, 'HP': 10, 'position': [3, 1], 'location': 'tech_hub', 'chocolate': 0}
        area_description = {
            'rows': 8,
            'columns': 7,
        ...
        actual = generate_chocolate(player, area_description)
        self.assertEqual(expected, actual)


    @patch('random.sample', side_effect=lambda population, k: [(0, 3)] if k == 1 else [(0, 3), (0, 0), (1, 3), (0, 2), (2, 0)])
    def test_generate_chocolate_student_lounge(self, _):
        player = {'name': 'Irene', 'level': 1, 'intelligence': 0, 'HP': 10, 'position': [3, 1], 'location': 'student_lounge',
                  'chocolate': 0}
        area_description = {
                    'rows': 10,
                    'columns': 7,
    ...
    @patch('random.sample', side_effect=lambda population, k: [(0, 3)] if k == 1 else [(8, 2), (7, 0), (10, 0), (0, 2), (4, 0)])
